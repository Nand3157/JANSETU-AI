...
- **Translation** — `POST /api/translate` (`{ text, targetLang, sourceLang }` → `translation`) renders a citizen's text in the other two languages, on the same governed prompt contract as intake: it may re-say the words, never answer, summarise or improve a report. Choosing a language in the recorder or the input-language selector translates what is already written. The box is what gets submitted; the words the citizen actually wrote are frozen as `verbatimText` / `verbatimLanguage` and shown as "as first written" on the request, so a translation can never erase the source. Switching back to the written language restores the exact original, and switching languages always re-renders from that original — never from the previous translation.
