...
export default function ProjectsPage() {
  ...
  useEffect(()=> { api("/api/projects/recommended").then((p:any)=> setProjects(p.projects||[])).catch(()=>{}); }, []);
  return (
        ...
        {projects.map((p,i)=> (
          <div key={p.projectId} className="rounded-[20px] bg-white border border-[#E5E7EB] p-5 flex flex-col">
            ...
            <div className="mt-3 flex gap-2">
              <span className="rounded-full bg-[#F8FAFC] border border-[#E5E7EB] px-2.5 py-1 text-xs">₹{(p.estimatedCost/1e7).toFixed(1)} Cr</span>
              <span className="rounded-full bg-[#F8FAFC] border border-[#E5E7EB] px-2.5 py-1 text-xs">{p.estimatedBeneficiaries?.toLocaleString()} beneficiaries</span>
            </div>
            <div className="mt-3 flex items-center gap-2">
              <Badge tone="ai">Candidate Recommendation</Badge><span className="text-xs text-[#5F6368]">Human review required</span>
            </div>
            <Link href={`/government/projects/${p.projectId}`} className="mt-4"><Button size="sm" className="w-full rounded-full gap-1.5">Review Project <ArrowRight className="h-3.5 w-3.5" /></Button></Link>
          </div>
        ))}
        {!projects.length && <div className="col-span-3 rounded-[20px] bg-white border border-[#E5E7EB] p-10 text-center text-sm text-[#5F6368]">No projects yet — generate from a hotspot in Overview.</div>}
      </div>
    </div>
  );
}
