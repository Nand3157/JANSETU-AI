  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-[#5F6368]">Top cards + Baseline → Target → Actual</p>
      </div>
      <div className="grid md:grid-cols-4 gap-4">
        {[
          ...
          <div key={l as string} className={`rounded-[20px] border border-[#E5E7EB] p-4 ${cls as string}`}>
            ...
            <div className="text-2xl font-semibold mt-1">{v as string}</div>
          </div>
        ))}
      </div>
      <div className="rounded-[20px] bg-white border border-[#E5E7EB] p-5">
        <div className="flex items-center justify-between">
          ...
          <Button size="sm" variant="secondary" className="rounded-full" onClick={load}>Load Impact</Button>
        </div>
        <div className="mt-4 space-y-3">
          {[
            ...
            <div key={metric as string} className="flex items-center justify-between rounded-xl border border-[#E5E7EB] px-4 py-3">
              <div><div className="text-sm font-medium">{metric as string}</div><div className="text-sm text-[#5F6368]">{vals as string}</div></div>
              <Badge tone={label==="Measured"?"verified":"estimate"}>{label as string}</Badge>
            </div>
          ))}
        </div>
        {impact && <div className="mt-4 rounded-xl bg-[#F8FAFC] border border-[#E5E7EB] p-3 text-xs">Observed: {impact.observed_changes?.join(", ")||"—"} · Estimated: {impact.estimated_impact?.[0]?.estimated} · {impact.limitations?.[0]}</div>}
        <div className="text-xs text-[#5F6368] mt-3 flex gap-2"><Badge tone="verified">Measured</Badge><Badge tone="estimate">Estimated</Badge><Badge tone="ai">Modeled</Badge></div>
      </div>
    </div>
  );
}
