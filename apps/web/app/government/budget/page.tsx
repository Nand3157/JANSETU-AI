  ...
  return (
    <div className="space-y-4 max-w-[960px]">
      ...
      <Card className="p-6 space-y-5">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <label className="text-sm">Optimization
            <select value={objective} onChange={e=> setObjective(e.target.value)} className="mt-1 w-full rounded-full border border-[#E5E7EB] bg-white px-3 py-2.5 text-sm">
              <option value="max_priority">Maximum Population Impact</option>
              <option value="max_beneficiaries">Maximum Beneficiaries</option>
              <option value="equity">Vulnerable Population</option>
              <option value="equity">Infrastructure Gap Reduction</option>
              <option value="equity">Balanced Development</option>
            </select>
          </label>
        ...
        <Button onClick={simulate} className="rounded-full">Simulate Portfolio</Button>
        {res && (
          <div className="rounded-[16px] bg-[#F8FAFC] border border-[#E5E7EB] p-4 space-y-2">
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div className="rounded-xl bg-white border border-[#E5E7EB] p-3"><div className="text-lg font-semibold">{res.unfunded_high_priority?.length}</div><div className="text-xs text-[#5F6368]">Unfunded High</div></div>
            </div>
            <div className="text-xs"><span className="font-medium">Trade-offs:</span> {res.trade_offs}</div>
            <div className="space-y-1.5">
              {res.selected_projects?.map((p:any)=> (
                <div key={p.projectId} className="rounded-xl bg-white border border-[#E5E7EB] p-2 flex justify-between text-xs"><span className="font-medium">{p.title}</span><span>₹{(p.estimatedCost/1e7).toFixed(1)}Cr</span></div>
              ))}
            </div>
          </div>
        )}
