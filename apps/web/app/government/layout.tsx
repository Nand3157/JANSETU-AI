...
export default function GovLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(()=> {
    ...
    if (!u || (!auth?.currentUser && u.role === "citizen")) { setMockRole("government"); setReady(true); return; }
    if (!u) { router.replace("/login"); return; }
    if (!isFirebaseConfigured() && u.role === "citizen") { setMockRole("government"); setReady(true); return; }
      ...
      <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="hidden md:flex items-center gap-2 rounded-full border border-[#E5E7EB] bg-[#F8FAFC] px-3 py-1.5 text-sm">
              <Search className="h-4 w-4 text-[#5F6368]" />
               <input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key === "Enter" && router.push(`/government/explorer?search=${encodeURIComponent(search)}`)} placeholder="Search requests, villages, projects..." className="bg-transparent outline-none placeholder:text-[#5F6368] w-[220px]" />
             </div>
             <div className="relative">
             <button aria-label="Show notifications" aria-expanded={noticeOpen} onClick={()=>{setNoticeOpen(v=>!v);setProfileOpen(false)}} className="h-9 w-9 grid place-items-center rounded-full border border-[#E5E7EB] bg-white hover:bg-[#F8FAFC]"><Bell className="h-4 w-4" /><span className="absolute right-0.5 top-0.5 h-2 w-2 rounded-full bg-[#D93025]" /></button>
             ...
             {profileOpen && <div className="absolute right-0 top-11 z-30 w-52 rounded-2xl border border-[#E5E7EB] bg-white p-2 shadow-xl"><Link href="/government/admin" onClick={()=>setProfileOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2 text-sm hover:bg-[#F8FAFC]"><Settings className="h-4 w-4" /> Settings</Link><button onClick={async()=>{await signOutMock(); router.replace("/login")}} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-[#D93025] hover:bg-red-50"><LogOut className="h-4 w-4" /> Sign out</button></div>}
             </div>
          </div>
        </div>
        ...
        {mobile && (
          <div className="lg:hidden bg-white border-b border-[#E5E7EB] px-3 py-3 space-y-1">
             {[...nav, ...admin].map(n=> (
               <Link key={n.href} href={n.href} onClick={()=> setMobile(false)} className={`flex items-center gap-2 px-3 py-3 rounded-xl text-sm ${isActive(n.href, ("exact" in n && n.exact === true) || undefined) ? "bg-[#E8F0FE] text-[#174EA6] font-semibold" : "bg-[#F8FAFC]"}`}><n.icon className="h-4 w-4" />{n.label}</Link>
             ))}
          </div>
        )}
