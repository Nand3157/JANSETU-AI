...
import { loadLastRequestId } from "@/lib/draft";

export default function SuccessPage() {
  const [requestId, setRequestId] = useState<string | null>(null);
  useEffect(()=> { setRequestId(loadLastRequestId() || null); }, []);
  const displayId = requestId || "JP-10483";
  const trackHref = requestId ? `/citizen/requests/${requestId}` : "/citizen/requests";
  return (
    <div className="min-h-[100vh] bg-[#F8FAFC] grid place-items-center p-6">
      <div className="w-full max-w-[480px] rounded-[24px] bg-white border border-[#E5E7EB] p-8 text-center">
        <div className="h-16 w-16 rounded-full bg-[#E6F4EA] text-[#188038] grid place-items-center mx-auto"><CheckCircle2 className="h-8 w-8" aria-hidden="true" /></div>
        ...
        <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#F8FAFC] border border-[#E5E7EB] px-4 py-2 text-sm"><span className="text-[#5F6368]">Request ID</span> <span className="font-semibold tabular-nums">{displayId}</span>{!requestId && <span className="text-xs text-amber-700">· demo</span>}</div>
        ...
        <Link href={trackHref} className="mt-6 block"><Button className="w-full rounded-full">Track My Request</Button></Link>
        <div className="text-xs text-[#5F6368] mt-3">Evidence-led · Human review required {requestId ? `· ${requestId}` : ""}</div>
        <Link href="/citizen/requests" className="text-xs text-[#174EA6] underline mt-2 inline-block">View all my requests</Link>
      </div>
    </div>
  );
}
