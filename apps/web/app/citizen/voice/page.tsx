    ...
    <div className="min-h-[100vh] bg-[#F8FAFC] flex flex-col">
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        ...
        <div className="mt-8 w-full max-w-[520px] rounded-[20px] bg-white border border-[#E5E7EB] p-5 text-left">
          <select value={lang} onChange={e=> setLang(e.target.value)} className="mb-4 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm">
        ...
        <button onClick={()=> router.push("/citizen/submit")} className="mt-6 text-xs text-[#5F6368] underline">Prefer to type instead</button>
      </div>
    </div>
  );
}
