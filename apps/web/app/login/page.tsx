...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setError(""); setNotice("");
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim()) && !/^(\+91[\s-]?)?[6-9]\d{9}$/.test(email.trim())) {
    ...
    try {
      if (isFirebaseConfigured()) {
        // Real Firebase Auth — role resolved from custom claims / users doc
        ...
        // Demo mode (no Firebase config): header-based demo session
        setMockRole(role);
          ...
          <form onSubmit={submit} noValidate className="mt-6 space-y-4">
            ...
            <label className="block">
              ...
              <div className="mt-1.5 relative">
                ...
                <button type="button" onClick={()=> setShow(v=>!v)} aria-label={show?"Hide password":"Show password"} className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 w-8 grid place-items-center rounded-full hover:bg-[#F5F5F4]">{show?<EyeOff className="h-4 w-4"/>:<Eye className="h-4 w-4"/>}</button>
              </div>
            </label>
