...
export function PhotoUploader({ onFile }: { onFile: (file: File | null, previewUrl?: string)=>void }) {
  ...
  function onChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f.size > 5 * 1024 * 1024) { alert("Photo must be ≤5MB"); return; }
    setName(f.name);
    if (preview) URL.revokeObjectURL(preview);
    ...
    <div className="space-y-2">
      ...
      {!preview ? (
        ...
        <div className="relative rounded-2xl overflow-hidden border border-slate-200 bg-white">
          ...
          <button aria-label="Remove photo" onClick={handleRemove} className="absolute top-2 right-2 h-8 w-8 rounded-full bg-black/60 text-white grid place-items-center"><X className="h-4 w-4" aria-hidden="true" /></button>
